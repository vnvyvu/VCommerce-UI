...
import {
	FrameMotionBox,
	MotionBox,
} from '../../components/motion/Motion.component';
import { registerStatusSelector } from '../../features/user/user.selector';
import { userActions } from '../../features/user/user.slice';
import {
	registerEmailSelector,
	registerNameSelector,
	registerPasswordSelector,
	registerRepasswordSelector,
	registerSelector,
} from './register.selector';
import { registerActions } from './register.slice';

function Register({ drawerOpen }) {
	...
	return (
		...
		>
			<Top
				...
				onAgreeTerms={(e) => setAgreeTerms(e.target.checked)}
			/>
	...
	return (
		...
		>
			...
			<FormControlLabel
				...
				control={<Checkbox onChange={onAgreeTerms} />}
			/>
		</FrameMotionBox>
	);
}
...
function Middle({ drawerOpen }) {
	return (
		...
		>
			...
			<InputName
				fullWidth
				selector={registerNameSelector}
				actionUpdate={registerActions.update}
			/>
			<InputEmail
				fullWidth
				selector={registerEmailSelector}
				actionUpdate={registerActions.update}
			/>
			<InputPassword
				fullWidth
				repassword
				selector={registerPasswordSelector}
				repasswordSelector={registerRepasswordSelector}
				actionUpdate={registerActions.update}
			/>
			<ButtonRegister />
		</FrameMotionBox>
...
function ButtonRegister() {
	...
	const { registerStatus, errorMsg } = useSelector(registerStatusSelector);
	const handleSubmitRegister = (e) => {
		e.preventDefault();
		const result = Object.entries(register).reduce(
			(result, [key, valueObj]) =>
				typeof valueObj !== 'string' &&
				(!valueObj.value || valueObj.helperText)
					? {
							...result,
							[key]: {
								helperText:
		...
		if (Object.keys(result).length)
			dispatch(registerActions.update(result));
		else {
	...
	return (
		<>
			<LoadingButton
				...
				startIcon={<i className='far fa-hand-point-right'></i>}
				sx={{
					height: '2rem',
					px: 10,
					py: 2.5,
			...
			>
				{registerStatus === 'pending' ? 'Registering...' : 'Submit'}
			</LoadingButton>
			{registerStatus === 'failed' && (
				<Alert severity='error' sx={{ height: '2.9rem', width: 1 }}>
					{errorMsg}
				</Alert>
			)}
		</>
	...
	return (
		<>
			...
			<ul>
				...
				<li>
					<Typography variant='body2' component='div'>
						<strong>Website</strong> refers to VCommerce, accessible
						from{' '}
						<a
